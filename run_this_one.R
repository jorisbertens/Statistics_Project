...
plot(cor_df$cor_abs, xlab = "Independent Variables", ylab = "Correlation to Unemployment Rate")
abline(0.15, 0)
...
df_regression2 <- as.data.frame( scale(df_regression))
...
#windows() 
plot(df_regression$Unemployment, df_regression$pred, main="Predicted vs Actual", 
     xlab="Actual", ylab="Predictions", pch=3, xlim = c(-2,3), ylim = c(-2,3), ) 
abline(a=0, b=1)

plot(predict(model),df_vfit$Unemployment, main = "Predicted vs Actual", xlab="predicted", ylab="actual")

plot(df_regression$Unemployment_log, df_regression$pred, main="Scatterplot Example",
     xlab="Ground truth", ylab="Estimations", pch=3, xlim = c(-2,2), ylim = c(-2,2)) 
abline(a=0,b=2)
...
#PCA
df_pca = filter_df
res.pca = prcomp(df_pca[-1], scale. = FALSE , graph = FALSE)
...
pca_df <- pca_df %>% select(PC1,PC2,PC3,PC4,PC5,PC6,PC7,PC8)
